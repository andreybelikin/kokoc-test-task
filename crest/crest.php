<?php
	...
	class CRest
	{
		...
		private static $prevErrorTime = 0;
				...
				try
				{
					do
					{
						...
						if(!empty($result[ 'error' ]))
						{
							...
							{
								$bExit = false;
								if((self::$errorCount > 9) || (self::$errorCount < 0))
								{
									self::$errorCount = 0;
								} 
								if(self::$errorCount == 0)
								{
									self::$prevErrorTime = time();
								}
								elseif(self::$errorCount > 0)
								{
									$deltaTime = time() - self::$prevErrorTime;
									$secSleep = intval(ceil(self::SLEEP_SEC[ self::$errorCount ]*1.25));
									if($deltaTime > $secSleep)
									{
										self::$errorCount--;
									}
								}
								sleep(self::SLEEP_SEC[ self::$errorCount ]);
								self::$errorCount++;
							}
						}
